...
from pyspark.sql.functions import avg, col, count, lag
...
if __name__ == "__main__":
    ...
    spark = (
        SparkSession.builder.appName("CalculateSessionMetricsOptimized")
    ...
    data_with_sessions = (
        df.withColumn(
            "end_timestamp", col("timestamp").cast("long") + (col("duration_ms") / 1000)
        )
        .withColumn(
            "time_diff",
            col("timestamp").cast("long") - lag("end_timestamp").over(window_spec),
        )
        .withColumn(
            "is_new_session",
            when(
                col("time_diff").isNull() | (col("time_diff") > session_gap), 1
            ).otherwise(0),
        )
        .withColumn("session_id", _sum("is_new_session").over(window_spec))
        .withColumn(
            "time_diff", when(col("is_new_session") == 1, 0).otherwise(col("time_diff"))
        )
    )

    # Calculate session-level metrics
    session_metrics = data_with_sessions.groupBy("user_id", "session_id").agg(
